...
function compare(p, s) {
    let bestRT = p.avgRT < s.avgRT ? "Priority Scheduling" : (s.avgRT < p.avgRT ? "SRTF Scheduling" : "Tie (Both Equal)");
    let bestWT = p.avgWT < s.avgWT ? "Priority Scheduling" : (s.avgWT < p.avgWT ? "SRTF Scheduling" : "Tie (Both Equal)");
    let bestTAT = p.avgTAT < s.avgTAT ? "Priority Scheduling" : (s.avgTAT < p.avgTAT ? "SRTF Scheduling" : "Tie (Both Equal)");
