...
function nonPreemptivePriority(processes) {
    let n = processes.length;
    let remaining = processes.map(p => p.burst);
    let isFinished = new Array(n).fill(0);
    let completed_count = 0;
    let current_time = 0;
    let timeline = [];
    ...
    while (completed_count < n) {
        let best_process = -1;
        for (let i = 0; i < n; i++) {
            if (processes[i].arrival <= current_time && !isFinished[i]) {
                if (best_process === -1 || processes[i].priority < processes[best_process].priority) {
                    best_process = i;
                } else if (processes[i].priority === processes[best_process].priority) {
                    if (processes[i].arrival < processes[best_process].arrival) {
                        ...
                        if(processes[i].arrival < processes[best_process].arrival){best_process = i;}
                    }
                }
            }
        }
        ...
        if (best_process === -1) {
            ...
            for(let i=0; i<parseInt(processes[best_process].burst); i++){
                timeline.push(processes[best_process].id);
            }
            processes[best_process].RT = current_time- processes[best_process].arrival;
            current_time+=processes[best_process].burst;
            processes[best_process].TAT = current_time - processes[best_process].arrival;
            processes[best_process].WT = processes[best_process].TAT - processes[best_process].burst;
            isFinished[best_process]=true;
            completed_count++;
